...
export default function SharkTankGame({ onClose }: Props) {
  ...
  const [view, setView] = useState<View>({ kind: "video" });
  const { speak, stop } = useVoice(!muted);
  const sfxRef = useRef<AudioContext | null>(null);

  const sfx = (freq: number, dur = 0.1, type: OscillatorType = "square", vol = 0.05) => {
    if (muted) return;
  ...
  // Skip intro video → go to intro node
  const startGame = () => {
    sfx(660, 0.15, "triangle");
    enterNode("intro");
  };
  ...
  const advance = () => {
    ...
    if (view.idx < view.beats.length - 1) {
      ...
      // Beats finished → either show choices for this node or jump to next
  ...
  const reset = () => { stop(); setView({ kind: "video" }); };
      ...
      >
        ...
        <div className="relative flex items-center justify-between border-b-[3px] border-[hsl(var(--ink))] bg-accent px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-stroke text-lg font-black tracking-wide text-white sm:text-2xl">
        ...
        {/* Stage */}
        <div className="relative min-h-[460px] px-4 pb-6 pt-4 sm:px-8">
